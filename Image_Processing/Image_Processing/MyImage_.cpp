...
HRESULT MyImage_::Load(_In_z_ LPCTSTR pszFileName) 
{
	...
	int h=m_CImage.GetHeight();
	int GrayOrColor=m_CImage.GetBPP ();
	...
	srcm_pBits = (BYTE***)new   BYTE**[3];
	for(int i=0;i<3;i++)  
	{ 
		m_pBits[i] = (BYTE**)new  BYTE*[h];   
		srcm_pBits[i] = (BYTE**)new  BYTE*[h];
	}   
	for   (int   i=0;   i<3;   i++)   
	{          
		for   (int   j=0;   j<h;   j++)         
	...
	if (GrayOrColor == 24)
	{
		for (int j = 0; j<h; j++)
		{
			for (int k = 0; k<w; k++)
			{
				...
				srcm_pBits[0][j][k] = psrc[j*nrow + k * 3];//B
				srcm_pBits[1][j][k] = psrc[j*nrow + k * 3 + 1];//G
				srcm_pBits[2][j][k] = psrc[j*nrow + k * 3 + 2];//R
			}
		}
	}
	if (GrayOrColor == 8)
	{
		for (int j = 0; j<h; j++)
		{
			for (int k = 0; k<w; k++)
			{
				...
				srcm_pBits[0][j][k] = psrc[j*nrow + k];//B
				srcm_pBits[1][j][k] = psrc[j*nrow + k];//G
				srcm_pBits[2][j][k] = psrc[j*nrow + k];//R
			}
		}
	}
